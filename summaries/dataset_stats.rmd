...
species_reconc <- species_reconc[-which(species_reconc$BS.plant_id == "40175425" & species_reconc$PL.scientific_name == "Rhamnus cathartica Linnaeus"),]
nrow(species_reconc)
...
I have dates and names parsed and resolved naming issues leaving 2047 samples. I then summarize the number of species and samples by project and by site with dates of measurements.
...
There are between 1 and 125 samples per species across all projects, or between 1 and 115 samples per species within one project. Each species is found at between 1 and 10 locations (sites). Sites range from having 1 to 25 species, and between 3 and 269 samples. There are a total of 120 species across all sites and projects.
...
nrow(final_metadata) #2047
nrow(spectra_trimmed) #205
length(unique(spectra_trimmed$sample_id)) #2047
...
trimmed_as_spectra_meta <- as_spectra(merged_meta_spectra, name_idx = 86, meta_idxs = c(1:88))
