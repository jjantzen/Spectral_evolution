...
The spectral data available was compiled by Shan Kothari (received October 21, 2020). Redoing this filtering/cleaning with the latest dataset from Shan Kothari (received January 14, 2022)

```{r read in spectral data}

#this is the preliminary version (cleaned by Shan)
#spectral_data <- readRDS("../data/intermediate_objects/all_spectra.rds")
spectral_data <- readRDS("../data/all_ref_and_traits.rds")
...
#make sure only submitted samples - remove statuses where deleted
#nrow(matched_meta)
...
nrow(plant_id_for_spectra) #2051 #redo is 2026
length(status_matched_meta$sample_id) #2052 - one more sample than have plant ids (redo is 2027)
...
status_matched_meta[which(duplicated(status_matched_meta$sample_id) == TRUE),c(9,10,12,13,19,20,21,22)] #sample_id = 44682364 (redo 44682364)
...
colnames(status_matched_meta) <- paste0("LS.", names(status_matched_meta)) #leaf samples
...
length(unique(species_reconc$BS.plant_id)) #1804 (redo 1793)
length(unique(species_reconc$LS.sample_id)) #2050 (redo 2025)
...
species_reconc$LS.sample_id[which(duplicated(species_reconc$LS.sample_id) == TRUE)][1] #41295307 (not in redo)

#41295307 sample id
#plant id 40175425 (not in redo)
...
#remove two samples with different trees (for now - revisited - get rid of Rhamnus and keep Acer) - Acer is good one, Rhamnus is bad one
#species_reconc <- species_reconc[-(which(species_reconc$BS.plant_id == "40175425" & species_reconc$PL.scientific_name == "Rhamnus cathartica Linnaeus")),] #omit because not in redo data and gets rid of all data
nrow(species_reconc)

#check a plant id example
species_reconc[which(species_reconc$BS.plant_id == "8553450"),c(1,2,22,23,84,86,87,95,176)]
species_reconc[which(species_reconc$BS.plant_id == "40175425"),c(1,2,22,23,84,86,87,95,176)]
...
Of 2052 samples of status "submitted", there are 2051 with plant IDs (this is resolved after removing duplicate samples). There are two samples in the leaf spectra dataframe with duplicate sample IDs. These were removed here leaving 2050 samples. After merging the dataframes, two different plants (different species names, different locations) with the same plant ID were identified. The correct one (Acer negundo) was kept (based on comment in PL dataframe and concordance of LS and BS and PL scientific names), leaving 2050 samples. 

After redoing with Shan's latest spectra, we have 2025 samples. 
...
There are 167 samples with names that need updating (will update all but those will have changes). Spectral dataframe will be updated with the PL.scientific_name column. 
...
#remove specific epithet "Linnaeus" for Quercus samples
...
I have dates and names parsed and resolved naming issues leaving 2047 samples (2022 when redoing with latest data). I then summarize the number of species and samples by project and by site with dates of measurements.
...
There are between 1 and 125 (120 when redoing) samples per species across all projects, or between 1 and 115 (110 when redoing) samples per species within one project. Each species is found at between 1 and 10 locations (sites). Sites range from having 1 to 25 species, and between 3 and 269 (or when redoing 260) samples. There are a total of 120 species across all sites and projects.
...
n2_locations <- aggregated_species_data %>% 
  melt(id = c(1:6)) %>% 
  dplyr::filter(n_locations > 2 & value == 1) %>% 
  dplyr::select(c(1:7)) 
...
sp_top11_sites <- aggregated_species_data %>% 
  melt(id = c(1:6)) %>% 
  dplyr::filter(variable %in% top_11_sites$BS.site_id) %>% 
  spread(variable, value) %>% 
...
summary_of_growth_forms <- growth_form_3[,-1] %>%  #start with the data frame
  ...
  dplyr::select(resp, everything(), -response) %>%  #reorder the columns with resp at the front, removing response 
  arrange(desc(resp))
...
This was not redone with the new spectral data. Using VASCAN, I resolved species names and retrieved higher order classifications for all species in VASCAN (ie not tropical species). One species was also a synonym (Berberis aquifolium Pursh) so I need to go back and fix that before running VASCAN to get higher classification (or at beginning before any summary stuff). 
...
This was not redone with new spectral data. There are 10 species that were sampled over a three month period or longer. There are 25 species that were sampled over a two month period or longer.
...
nrow(spectra_trimmed) #(2022)
#add spectra to metadata itself
merged_meta_spectra <- merge(final_metadata, spectra_trimmed, by.x = "LS.sample_id", by.y = "sample_id")
nrow(merged_meta_spectra) #(2022)

#there are some duplicate spectra for the same sample numbers - check this out
nrow(final_metadata) #2047 (2022)
nrow(spectra_trimmed) #205 (2022)
length(unique(spectra_trimmed$sample_id)) #2047 (2022)
nrow(spec_as_df) #2255 (2026)

dup_spectra <- merged_meta_spectra$LS.sample_id[which(duplicated(merged_meta_spectra$LS.sample_id) == TRUE)] #no dups in latest data
...
#add column for species names in right format
merged_meta_spectra <- merged_meta_spectra %>% 
  unite(species_names, genus.x, specific_epithet, subspecific, sep = "_", na.rm = TRUE)
  
#make spectra back into spectral object
trimmed_as_spectra <- as_spectra(spectra_trimmed, name_idx = 1, meta_idxs = c(2:81))

#with different metadata
trimmed_as_spectra_meta <- as_spectra(merged_meta_spectra, name_idx = 86, meta_idxs = c(1:164))
colnames(merged_meta_spectra)

#minimal metadata
merged_meta_spectra_min <- merged_meta_spectra[,c(1,3,8,19,20,21,27,28,30:33,86:90,94,165:2165)]
ncol(merged_meta_spectra)
trimmed_as_spectra_meta_min <- as_spectra(merged_meta_spectra_min, name_idx = 13, meta_idxs = c(1:18))
colnames(merged_meta_spectra_min)
...
saveRDS(trimmed_as_spectra_meta, "../data/tidy/trimmed_spectra_with_meta.rds")
saveRDS(trimmed_as_spectra_meta_min, "../data/tidy/trimmed_spectra_with_minimal_meta.rds")
write.csv(merged_meta_spectra, "../data/tidy/merged_meta_spectra_trimmed.csv", row.names = FALSE)

unique(merged_meta_spectra$species)


merged_meta_spectra$species_names[which(is.na(merged_meta_spectra$species))]
...
I need to figure out why those eight (four numbers) spectra are duplicated. I asked Florence and don't remember what she said. I have left them in for now. 

For the manuscript(s), I need summary data for the projects so below I extract names of projects, number of species and samples per project, date ranges for projects, and sampling locations and taxonomic focus for each project. 

``` {r get project summaries}

#group by project to get number of samples and species per project with P/A of locations per species
#may need to adjust across numbers = just the columns of locations
project_summary <- final_metadata %>% 
  group_by(LS.project) %>% 
  summarise(n_ind = n(), n_sp = length(unique(PL.scientific_name)), month_range = paste0(min(month_measured), "-", max(month_measured)),  n_locations = length(unique(BS.site_id)), across(c(35:83), max)) #, n_species = n(unique(species))

#get list of geographic regions by project
#geography = list(paste0(unique(BS.site_id), sep = ","))

project_geography <- final_metadata %>% 
  dplyr::select(LS.project, BS.site_id) %>% 
  group_by(LS.project) %>% 
  summarize(geography = unique(BS.site_id))

#get list of species by project
project_species <- final_metadata %>% 
  dplyr::select(LS.project, PL.scientific_name, genus, specific_epithet, subspecific) %>% 
  group_by(LS.project) %>% 
  summarize(species = unique(PL.scientific_name))

colnames(final_metadata)


saveRDS(project_summary, "../data/tidy/project_summary.rds")
write.csv(project_summary, "../data/tidy/summary_of_projects.csv")

saveRDS(project_species, "../data/tidy/project_species.rds")
write.csv(project_species, "../data/tidy/summary_of_project_species.csv")

saveRDS(project_geography, "../data/tidy/project_geography.rds")
write.csv(project_geography, "../data/tidy/summary_of_project_geography.csv")
