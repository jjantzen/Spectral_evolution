...
#make lists of traits
traits <- c("woody", "gf", "myc", "persistence", "habitat", "Raunk") 
#uses myc as single trait with ?four? levels? or just two (AM/EM)

#may need to trim tree again if omitting any ErM or NM species
...
#run loop over 100 trees - make a function
for (i in 1:length(trees)){
  #run for each of 4 models of evolution
  for (j in 1:length(models)){
    #specify model - would it work to replace trait ~ myc * woody * persistence with "expression" in function
    model <- mvgls(trait ~ myc * woody * persistence, data=data_spectra, tree=trees[i], model=models[j], error = TRUE)
    #assign output
    if (j == 1){
    ...
    #save model itself
    saveRDS(model, paste0("./models/myc.woody.persistence_model_", models[j], "_iteration", i, ".rds"))
  }
}

saveRDS(df_output, "./model_parameters_myc.woody.persistence.rds")
