...
remove <- c("Vitis riparia", "Claytonia perfoliata")

#unique(new_spectra$meta$species_names[-which(new_spectra$meta$species_names %in% myc_data$species)])


refls_sub <- new_spectra %>% 
  subset(!(new_spectra$meta$species_names %in% remove)) #left with 1640 samples

num_sp <- unique(refls_sub$meta$species_names) #91 species
...
write.csv(r, file = "./analysis/plsda/categories_count_4cat.csv")
...
#4state: best is 19 (12) then 20 (8)
## pick_pls function
...
pdf("./output/plsda_kappa_comparison_finding_ncomp_4cat.pdf", height = 8, width = 10)
...
####JJ: 4cat: 16 is lowest not different from a
...
saveRDS(train_fit, "./analysis/plsda/PLSDA_fast_train_T6_16comps_4cat.rds")
saveRDS(probis, "./analysis/plsda/PLSDA_fast_T6_probis_16comps_4cat.rds")
saveRDS(confus, "./analysis/plsda/PLSDA_fast_T6_confus_16comps_4cat.rds")
...
write.csv(prob_mean,"./analysis/plsda/PLSDA_T6_probmean_16comps_4cat.csv")
...
jpeg("./output/plsda_id_probability_by_myc_plot_round2_16comp_4cat.jpg", height = 5, width = 8, res = 500, units = "in")
...
write.csv(tab_mean,"./analysis/plsda/PLSDA_all_confumean_16comps_4cat.csv")
...
write.csv(tabs_perc,"./analysis/plsda/PLSDA_all_confuperc_16comps_4cat.csv")
...
#make plot
jpeg("./output/plsda_confusion_matrix_myc_16comps_4cat.jpg", height = 6, width = 6, res = 500, units = "in")
par(mar=c(5,8,8,8), fig=c(0.05,1,0,0.95), new=F)
corrplot::corrplot(tabs_perc, is.corr = F, col = c(coul), tl.col = 1, cl.pos = "n",
                   tl.offset =1, tl.cex = 0.9, tl.srt = 70,
                   addCoef.col ='black', number.font = 1, number.cex = 0.9, addCoefasPercent = T, na.label = " ") 

mtext("Prediction",2, line=8, cex=1.2)
mtext("Reference",3, line = 8, cex=1.2)
mtext("Classification accuracy (%)", line = -20.5, cex=0.9)
...
write.csv(mm2, file = "./analysis/plsda/plsda_loadings_16comps_4cat.csv")
...
ggsave("./output/loadings_16comp_4cat.pdf", width = 12, height =8, dpi = 600 )
