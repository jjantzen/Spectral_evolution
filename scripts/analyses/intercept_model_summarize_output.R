...
#for file in folder
#files <- list.files("./analysis/intercept_models/parameters/", pattern=NULL, all.files=FALSE, full.names=TRUE)

files <- list.files("./analysis/intercept_models/myc_dataset/parameters/", pattern=NULL, all.files=FALSE, full.names=TRUE)
...
#save output
saveRDS(output_sorted, "./analysis/intercept_models/myc_dataset/summarized_parameters_intercept_models_mycdataset.rds")

nrow(output_sorted)
GIC(intercept_model_BM_iteration1)
...
saveRDS(model_parameters_gics_filled, "./analysis/intercept_models/myc_dataset/summarized_best_models_100trees_mycdataset.rds")
...
saveRDS(summary, "./analysis/intercept_models/myc_dataset/summary_intercept_models_mycdataset.rds")
...
model_parameters_gics_filled <- readRDS("./analysis/intercept_models/myc_dataset/summarized_best_models_100trees_mycdataset.rds")
...
long_all_trees <- pivot_longer(summarized_parameters_intercept_models_mycdataset[c(1,4,7,10)], cols = c(2:4))
...
pdf("./output/model_assessment/intercept_92sp_parameter_distributions.pdf", width = 10, height = 8)
par(mfrow=c(2,1))
best_only
all_models
dev.off()

jpeg("./output/model_assessment/intercept_92sp_parameter_distributions.jpg", width = 6, height = 6, units = "in", res = 500)
