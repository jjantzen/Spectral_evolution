...
new_spectra <- readRDS("./data/for_analysis/brightness_normalized_sample_all_taxa.rds")
...
num_sp <- unique(new_spectra$species_names) #2022 samples
...
remove <- c("Chamaedaphne calyculata", "Claytonia perfoliata", "Eriophorum vaginatum", "Kalmia angustifolia", "Populus deltoides", "Rhododendron groenlandicum", 
            "Salix alba", "Salix interior", "Typha angustifolia")

refls_sub <- new_spectra %>% 
  subset(!(new_spectra$species_names %in% remove)) #left with 1640 samples

num_sp <- unique(refls_sub$species_names) #91 species

refls_sub_df <- as.data.frame(pivot_wider(refls_sub, id_cols = c(sample_name, species_names), names_from = name))
str(refls_sub_df)

#drop groups with very low sampling (3< indvd) and qusp - not needed (it's by group not by species)
# few <- refls_sub_df %>%
...
#add myc data
for (i in 1:nrow(refls_sub_df)){
  refls_sub_df$myc[i] <- myc_data$myc[which(myc_data$species == refls_sub_df$species_names[i])]  
}
...
write.csv(r, file = "./analysis/plsda/bright_norm_categories_count.csv")
...
#most: around 31 (12 times, while next top is 32 with 7)
#tried again: 20, next best is 21
#third time: 26, then 20
#for bright norm: 25 (7) and 29 (6)
...
#most: 20 is top
#br norm: 19 is top (10) then 25 (10)
...
pdf("./output/plsda_kappa_comparison_finding_ncomp_bright_norm.pdf", height = 8, width = 10)
...
####JJ: Bright norm: 19 is same as 20 which is lowest with a - redo other with 19 too
...
saveRDS(train_fit, "./analysis/plsda/PLSDA_fast_train_T6_19comps_bright_norm.rds")
saveRDS(probis, "./analysis/plsda/PLSDA_fast_T6_probis_19comps_bright_norm.rds")
saveRDS(confus, "./analysis/plsda/PLSDA_fast_T6_confus_19comps_bright_norm.rds")
...
write.csv(prob_mean,"./analysis/plsda/PLSDA_T6_probmean_19comps_bright_norm.csv")
...
jpeg("./output/plsda_id_probability_by_myc_plot_round2_19comp_bright_norm.jpg", height = 5, width = 8, res = 500, units = "in")
...
write.csv(tab_mean,"./analysis/plsda/PLSDA_all_confumean_19comps_bright_norm.csv")
...
write.csv(tabs_perc,"./analysis/plsda/PLSDA_all_confuperc_19comps_bright_norm.csv")

# #plot it
# tab_mean <- read.csv("./analysis/plsda/PLSDA_all_confumean_31comps.csv", row.names = 1)
# tabs_perc <- read.csv("./analysis/plsda/PLSDA_all_confuperc_31comps.csv", row.names = 1)

#make plot
jpeg("./output/plsda_confusion_matrix_AM_EM_bright_norm_19comp.jpg", height = 5, width = 5, res = 500, units = "in")
par(fig=c(0,1,0,1), new=F)
corrplot::corrplot(tabs_perc, is.corr = T, col = c(coul, coul), method = "circle", tl.col = 1, cl.pos = "n",
...
write.csv(mm2, file = "./analysis/plsda/plsda_loadings_19comps_bright_norm.csv")
...
ggsave("./output/loadings_19comp_bright_norm.pdf", width = 12, height =8, dpi = 600 )
