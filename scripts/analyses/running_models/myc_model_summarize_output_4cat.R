...
files <- list.files("./analysis/myc_models/98sp_4cat/parameters/", pattern=NULL, all.files=FALSE, full.names=TRUE)
...
#save output
saveRDS(output_sorted, "./analysis/myc_models/98sp_4cat/summarized_parameters_myc_models_98sp.rds")

output_sorted <- readRDS("./analysis/myc_models/98sp_4cat/summarized_parameters_myc_models_98sp.rds")
...
saveRDS(best_models, "./analysis/myc_models/98sp_4cat/best_models_myc_models_98sp.rds")
best_models <- readRDS("./analysis/myc_models/98sp_4cat/best_models_myc_models_98sp.rds")
...
saveRDS(summary, "./analysis/myc_models/98sp_4cat/summary_myc_models_98sp.rds")
...
best_only <- ggplot(best_models, aes(best_model, parameter))+ #[-which(best_models$best_model %in% c("BM", "lambda")),]
  geom_boxplot()+
  xlab("Model")+
  ylab("Parameter value")+
...
pdf("./output/model_assessment/myc_only_98sp_parameter_distributions_4cat.pdf", width = 10, height = 8)
par(mfrow=c(2,1))
best_only
all_models
dev.off()

jpeg("./output/model_assessment/myc_only_98sp_parameter_distributions_4cat.jpg", width = 6, height = 6, units = "in", res = 500)
