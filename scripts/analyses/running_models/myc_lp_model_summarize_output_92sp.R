...
#for file in folder
files <- list.files("./analysis/myc_lp_interaction_models/parameters/", pattern="model_parameters", all.files=FALSE, full.names=TRUE)
...
#save output
saveRDS(output_sorted, "./analysis/myc_lp_interaction_models/summarized_parameters_myc_lp_models_92sp_binary.rds")

output_sorted <- readRDS("./analysis/myc_lp_interaction_models/summarized_parameters_myc_lp_models_92sp_binary.rds")
...
saveRDS(best_models, "./analysis/myc_lp_interaction_models/best_models_myc_lp_models_92sp_binary.rds")
best_models <- readRDS("./analysis/myc_lp_interaction_models/best_models_myc_lp_models_92sp_binary.rds")
...
saveRDS(summary, "./analysis/myc_lp_interaction_models/summary_myc_lp_models_92sp_binary.rds")

summary <- readRDS("./analysis/myc_lp_interaction_models/summary_myc_lp_models_92sp_binary.rds")
...
saveRDS(stats, "./analysis/myc_lp_interaction_models/stats_myc_lp_models_92sp_binary.rds")

stats <- readRDS("./analysis/myc_lp_interaction_models/stats_myc_lp_models_92sp_binary.rds")
...
mean(best_models_ou$tree_height[c(1:14,16)])
sd(best_models_ou$tree_height[c(1:14,16)])
nrow(best_models_ou)
mean(best_models_ou$halflife[c(1:14,16)])
sd(best_models_ou$halflife[c(1:14,16)])

mean(best_models_ou$parameter[c(1:14,16)])
sd(best_models_ou$parameter[c(1:14,16)])
...
pdf("./output/model_assessment/myc_lp_92sp_parameter_distributions_binary.pdf", width = 10, height = 8)
par(mfrow=c(2,1))
best_only
all_models
dev.off()

jpeg("./output/model_assessment/myc_lp_92sp_parameter_distributions_binary.jpg", width = 6, height = 6, units = "in", res = 500)
