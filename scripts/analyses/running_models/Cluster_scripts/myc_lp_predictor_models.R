...
for (j in 1:length(models)){
  model <- mvgls(spectra ~ myc*lp, data = data_spectra, tree = trees, model=models[j], error = TRUE)
  #assign output
  ...
  #save model itself
  saveRDS(model, paste0("/home/jjantzen/scratch/R/output/myc_lp_models/92sp_binary/myc_lp_model_92sp_", models[j], "_iteration", taskid, ".rds"))
}

saveRDS(df_output, paste0("/home/jjantzen/scratch/R/output/myc_lp_models/92sp_binary/model_parameters_myc_lp_92sp_iteration", taskid, ".rds"))
